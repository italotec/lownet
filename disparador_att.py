...
def enviar_template(lead, phone_number_id, token, log_enabled=True):
    ...
    try:
        ...
        # Importante: mesmo usando -q, ainda precisa logar pra não reenviar depois
...
def modo_envio(random_mode=False, quantidade=None):
    bms = carregar_bms()
    if not bms:
    ...
    if not templates:
        print("❌ Essa BM não tem templates cadastrados.")
        return

    # CSV precisa ter: telefone, nome (opcional)
    leads = pd.read_csv("base10pra100k.csv")

    if 'telefone' not in leads.columns:
        print("❌ O CSV precisa ter a coluna 'telefone'.")
        return
    ...
    with open(LOG_FILE, "r", encoding="utf-8") as f:
        enviados = set(line.strip() for line in f if line.strip())
    ...
    # === NOVO: limitar quantidade após filtrar (e após random, se usado) ===
    if quantidade is not None:
        if quantidade <= 0:
            print("❌ A quantidade (-q/--quantidade) deve ser maior que zero.")
            return
        leads_filtrados = leads_filtrados.head(int(quantidade)).reset_index(drop=True)

    num_templates = len(templates)
    total_leads = len(leads_filtrados)

    if total_leads == 0:
        print("✅ Nenhum lead novo para enviar (todos já estão no sent_log).")
        return
    ...
    if quantidade is not None:
        print(f"📌 Limite aplicado (-q): {quantidade}")

    with ThreadPoolExecutor(max_workers=1) as executor:
        executor.map(
            lambda lead: enviar_template(lead, phone_number_id, token, log_enabled=True),
...
if __name__ == "__main__":
    ...
    parser.add_argument('--random', '-r', action='store_true', help='Enviar mensagens em ordem aleatória')
    parser.add_argument('--quantidade', '-q', type=int, default=None, help='Limitar quantidade de mensagens a enviar (ex: -q 2000)')
    args = parser.parse_args()

    if args.cadastrar:
        cadastrar_bm()
    else:
        modo_envio(random_mode=args.random, quantidade=args.quantidade)
